...
class CircularFixedList<T extends { id: unknown }> {
  ...
  constructor(items: T[]) {
    items.forEach((item, i) => {
      ...
      if (next == null) {
        throw new Error(`Could not find next for ${i}`);
      }
      const prev = items[i == 0 ? items.length - 1 : i - 1];
      if (prev == null) {
        throw new Error(`Could not find prev for ${i}`);
      }
      this.buffer.push({
        current: item,
        prev,
...
for await (const line of iterateLines("02.txt")) {
  ...
  if (opponentRule !== ownRule) {
    const hasWinningShape = ownRule.next.id === opponentRule.current.id;
    if (hasWinningShape) {
      totalScorePart1 += 6;
    }
  } else {
...
console.log(`Part1: ${totalScorePart1}`);
...
for await (const line of iterateLines("02.txt")) {
  ...
  const opponentRule = gameRules.getItem((shape) =>
    shape.symbols.includes(opponentShapeSymbol)
  );

  let ownShape: Shape;
  switch (outcome) {
    case Outcome.DRAW: {
      ownShape = opponentRule.current;
      totalScorePart2 += 3;
      break;
    }
    case Outcome.LOSS: {
      ownShape = opponentRule.next;
      totalScorePart2 += 0;
      break;
    }
    case Outcome.WIN: {
      ownShape = opponentRule.prev;
      totalScorePart2 += 6;
      break;
    }
  }
  totalScorePart2 += ownShape.score;
}

console.log(`Part2: ${totalScorePart2}`);
